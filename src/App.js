...
// Calcula importe: primero precio_custom, luego tarifario
function getImporte(row, circInfo, tarifario) {
  if (row.precio_custom != null && row.precio_custom > 0) {
    const m = row.moneda_custom || 'MXN'
    return m === 'USD'
      ? { mxn: 0, usd: row.precio_custom, found: true, custom: true }
      : { mxn: row.precio_custom, usd: 0, found: true, custom: true }
  }
  ...
  return match.moneda === 'USD'
    ? { mxn: 0, usd: total, found: true, custom: false }
    : { mxn: total, usd: 0, found: true, custom: false }
}
...
function Dashboard({ session }) {
      ...
      if (circs && circs.length > 0) {
        ...
        const full = circs.map((c) => ({
          ...
          rows: (allRows || []).filter((r) => r.circuit_id === c.id).map((r) => ({
            ...r, fecha: r.fecha ? new Date(r.fecha) : null,
          })),
        }))
        setCircuits(full)
  ...
  if (dataLoading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f1eb' }}>
      <div style={{ textAlign: 'center' }}><Spinner /><div style={{ marginTop: 12, color: '#8a8278', fontSize: 13 }}>Cargando datos...</div></div>
    </div>
  )
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", background: '#f5f1eb', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        {sidebarOpen && (
          <aside style={{ width: 248, background: '#12151f', borderRight: '1px solid rgba(255,255,255,.07)', overflowY: 'auto', flexShrink: 0, position: 'sticky', top: 54, height: 'calc(100vh - 54px)' }}>

            {/* Vista general */}
            <SbItem label="📊 Todos los circuitos" count={circuits.length} active={view.type === 'all'} onClick={() => setView({ type: 'all' })} />
            <SbItem label="📈 Resultados globales" count="" active={view.type === 'resultados_all'} onClick={() => setView({ type: 'resultados_all' })} />
            <SbDivider />

            {sortedMonths.map((mk) => (
              <div key={mk}>
                <div style={{ padding: '8px 16px 2px', fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, color: 'rgba(255,255,255,.3)' }}>{mk}</div>
                ...
                <SbItem label="📈 Resultados del mes" count="" active={view.type === 'resultados_mes' && view.monthKey === mk} onClick={() => setView({ type: 'resultados_mes', monthKey: mk })} indent />
                {monthMap[mk].map((c) => {
                  ...
                  return (
                    <div key={c.id} onClick={() => { setView({ type: 'circuit', circuitId: c.id }); setActiveTab('cxp'); setFilters({ tipo: 'ALL', cat: 'ALL', pago: 'ALL', fecha: '' }) }}
                      style={{ padding: '5px 16px 5px 32px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7, borderLeft: `3px solid ${view.circuitId === c.id ? '#b8952a' : 'transparent'}`, background: view.circuitId === c.id ? 'rgba(184,149,42,.1)' : 'transparent' }}>
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
          ...
          {view.type === 'resultados_mes' && <ResultadosMesView mk={view.monthKey} circuits={monthMap[view.monthKey] || []} tarifario={tarifario} TC={TC} />}
          {view.type === 'resultados_all' && <ResultadosGlobalView circuits={circuits} monthMap={monthMap} sortedMonths={sortedMonths} tarifario={tarifario} TC={TC} />}
          {view.type === 'circuit' && activeCircuit && (
            <CircuitDetail circ={activeCircuit} tarifario={tarifario} TC={TC} activeTab={activeTab} setActiveTab={setActiveTab}
              ...
              saveProv={saveProv} saveImporte={saveImporte}
...
//  RESULTADOS
// ══════════════════════════════════════════
function ResultadosMesView({ mk, circuits, tarifario, TC }) {
  return (
    <div>
      <h2 style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: 26, marginBottom: 4 }}>📈 Resultados — {cap(mk)}</h2>
      <p style={{ color: '#8a8278', fontSize: 13, marginBottom: 20 }}>{circuits.length} circuito{circuits.length !== 1 ? 's' : ''}</p>
      <ResultadosAgregados circuits={circuits} tarifario={tarifario} TC={TC} showPerCircuit />
    </div>
  )
}

function ResultadosGlobalView({ circuits, monthMap, sortedMonths, tarifario, TC }) {
  return (
    <div>
      <h2 style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: 26, marginBottom: 4 }}>📈 Resultados Globales</h2>
      <p style={{ color: '#8a8278', fontSize: 13, marginBottom: 20 }}>{circuits.length} circuitos · {sortedMonths.length} meses</p>
      <ResultadosAgregados circuits={circuits} tarifario={tarifario} TC={TC} title="Resumen total" />
      {sortedMonths.map((mk) => (
        <div key={mk} style={{ marginBottom: 32 }}>
          <div style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: 18, fontWeight: 700, margin: '24px 0 12px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '2px solid #ece7df', paddingBottom: 8 }}>
          ...
          <ResultadosAgregados circuits={monthMap[mk]} tarifario={tarifario} TC={TC} showPerCircuit />
        </div>
      ))}
    </div>
  )
...
function ResultadosAgregados({ circuits, tarifario, TC, title, showPerCircuit }) {
  const ci = {}, cc = {}, rd = {}
  ...
  circuits.forEach((circ) => {
    circ.rows.forEach((r) => {
      ...
      if (r.paid) { rd[cat].paidMXN += mxn; rd[cat].paidUSD += usd; tPMXN += mxn; tPUSD += usd }
      else { rd[cat].pendMXN += mxn; rd[cat].pendUSD += usd; tNMXN += mxn; tNUSD += usd }
    })
  })
  ...
  const H = ({ t }) => <h3 style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: 15, marginBottom: 12, paddingBottom: 8, borderBottom: '2px solid #ece7df' }}>{t}</h3>
  ...
  return (
    <div>
      {title && <p style={{ fontWeight: 700, fontSize: 14, marginBottom: 12, color: '#8a8278' }}>{title}</p>}
      <KPIGrid items={[
        { cls: 'gold', label: 'Circuitos', val: circuits.length },
        { cls: 'forest', label: '✅ Pagado MXN', val: fmtMXN(tPMXN), sub: fmtUSD(tPUSD) + ' USD' },
        { cls: 'rust', label: '⏳ Pendiente MXN', val: fmtMXN(tNMXN), sub: fmtUSD(tNUSD) + ' USD' },
        { cls: 'sky', label: 'Total Costos', val: fmtMXN(tc2) },
      ]} />
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <Card>
          <H t="📥 Ingresos (T-Venta)" />
        ...
        <Card>
          <H t="📤 Costos (Tarifario)" />
          {tc2 === 0 ? <p style={{ color: '#8a8278', fontSize: 12 }}>Sin tarifario capturado.</p>
      ...
      {tc2 > 0 && (
        <Card>
          <H t="📊 Distribución por Categoría" />
      ...
      <Card>
        <H t="💳 Pagado / Pendiente por Rubro" />
      ...
      {ti > 0 && tc2 > 0 && (
        <Card>
          <H t="💰 Margen" />
          <Row label="Total Ingresos" val={fmtMXN(ti)} color="#1e5c3a" />
          <Row label="Total Costos" val={fmtMXN(tc2)} color="#b83232" />
          <Row label="Margen Bruto" val={`${fmtMXN(mrg)} (${((mrg / ti) * 100).toFixed(1)}%)`} color={mrg >= 0 ? '#1e5c3a' : '#b83232'} bold />
        </Card>
      )}
      {showPerCircuit && circuits.length > 0 && (
        <Card>
          <H t="🗂 Desglose por Circuito" />
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead><tr style={{ background: '#12151f', color: '#fff' }}>
                {['Circuito', 'Tour Leader', 'Servicios', 'Costo MXN', 'Costo USD', '% Pagado'].map((h) => (
              ...
              <tbody>
                {circuits.map((circ) => {
                  ...
                  circ.rows.forEach((r) => { const { mxn, usd } = getImporte(r, circ.info, tarifario); cMXN += mxn; cUSD += usd })
                  const pct = circ.rows.length > 0 ? Math.round((paid / circ.rows.length) * 100) : 0
                  return (
                    <tr key={circ.id} style={{ borderBottom: '1px solid #ece7df' }}>
                      <td style={{ padding: '8px 10px', fontWeight: 600, fontSize: 11 }}>{circ.id.split('-').slice(-3).join('-')}</td>
                      <td style={{ padding: '8px 10px', fontSize: 11 }}>{circ.info?.tl || '—'}</td>
                      <td style={{ padding: '8px 10px' }}>{circ.rows.length}</td>
                      <td style={{ padding: '8px 10px', fontWeight: 700 }}>{cMXN > 0 ? fmtMXN(cMXN) : '—'}</td>
                      <td style={{ padding: '8px 10px', fontWeight: 700, color: '#1565a0' }}>{cUSD > 0 ? fmtUSD(cUSD) : '—'}</td>
                      <td style={{ padding: '8px 10px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                          <div style={{ flex: 1, height: 6, background: '#ece7df', borderRadius: 3, overflow: 'hidden', minWidth: 60 }}>
...
function SbItem({ label, count, active, onClick, indent }) {
  return (
    <div onClick={onClick} style={{ padding: `6px ${indent ? 24 : 16}px`, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderLeft: `3px solid ${active ? '#e0c96a' : 'transparent'}`, background: active ? 'rgba(184,149,42,.1)' : 'transparent' }}>
...
function TarifarioEditor({ tarifario, circuits, tarFileRef, onTarFile, onSave, onCancel, saving }) {
  const [rows, setRows] = useState(() => {
    ...
    circuits.forEach((c) => c.rows.forEach((r) => {
      ...
      if (p && !seen.has(p.toUpperCase())) { seen.add(p.toUpperCase()); out.push({ proveedor: p, tipo_servicio: r.clasificacion || 'HOSPEDAJE', precio: 0, moneda: 'MXN', dias_credito: 30, notas: '' }) }
    }))
    return out
  })
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, alignItems: 'center' }}>
        ...
        <span style={{ fontSize: 11, color: '#aaa' }}>Compartido por todo el equipo.</span>
      </div>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead><tr style={{ background: '#12151f', color: '#fff' }}>
...
function CircuitCards({ circs, tarifario, TC, onSelect }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(270px,1fr))', gap: 14 }}>
      {circs.map((c) => {
        let mxn = 0, usd = 0
        c.rows.forEach((r) => { const { mxn: m, usd: u } = getImporte(r, c.info, tarifario); mxn += m; usd += u })
        const paid = c.rows.filter((r) => r.paid).length
        ...
        return (
          <div key={c.id} onClick={() => onSelect(c.id)} style={{ background: '#fff', borderRadius: 12, padding: 16, boxShadow: '0 2px 16px rgba(18,21,31,.07)', borderTop: `3px solid ${allPaid ? '#52b788' : '#d8d2c8'}`, cursor: 'pointer', transition: 'all .2s' }}>
            ...
            <div style={{ height: 4, background: '#ece7df', borderRadius: 2, overflow: 'hidden', marginBottom: 8 }}><div style={{ height: '100%', width: pct + '%', background: '#52b788', borderRadius: 2 }} /></div>
...
// ── Circuit Detail ──
function CircuitDetail({ circ, tarifario, TC, activeTab, setActiveTab, F, setFilters, filteredRows, togglePaid, setFechaPago, setNota, saveProv, saveImporte, onDelete }) {
  ...
  return (
    <div>
      ...
      {activeTab === 'resultados' && <ResultadosAgregados circuits={[circ]} tarifario={tarifario} TC={TC} />}
    </div>
  )
}

// ── CxP Panel con edición de proveedor (solo del tarifario) e importe ──
function CxPPanel({ circ, tarifario, F, setFilters, filteredRows, togglePaid, setFechaPago, setNota, saveProv, saveImporte }) {
  const [editCell, setEditCell] = useState(null) // {rowId, field: 'prov'|'importe'}
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} style={{ borderBottom: '1px solid #ece7df', background: r.paid ? '#f0faf4' : 'transparent' }}>
                    ...
                    {/* ── Proveedor: solo opciones del tarifario ── */}
                    <td style={{ padding: '8px 10px', minWidth: 160 }}>
                      {isEditProv ? (
                        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                          <select value={editVal} onChange={(e) => setEditVal(e.target.value)} autoFocus
                            style={{ border: '1px solid #b8952a', borderRadius: 5, padding: '3px 7px', fontSize: 12, fontFamily: 'inherit', background: '#fff', maxWidth: 160 }}>
                          ...
                          <button onClick={cancelEdit} style={{ background: 'none', border: 'none', color: '#aaa', cursor: 'pointer', fontSize: 14 }}>✕</button>
                        </div>
                      ) : (
                        <div>
                          <span onClick={() => startEdit(r.id, 'prov', r)} style={{ fontWeight: 600, fontSize: 12, cursor: 'pointer', borderBottom: '1px dotted #b8952a', color: '#12151f' }} title="Clic para cambiar proveedor">
                    ...
                    {/* ── Importe MXN editable ── */}
                    <td style={{ padding: '8px 10px', minWidth: 110 }}>
                      {isEditImp ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          <input type="number" value={editVal} onChange={(e) => setEditVal(e.target.value)} placeholder="Importe"
                            style={{ border: '1px solid #b8952a', borderRadius: 5, padding: '3px 7px', fontSize: 12, fontFamily: 'inherit', width: 100 }} autoFocus />
                          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                    ...
                    <td style={{ padding: '8px 10px', minWidth: 90 }}>
                      <span onClick={() => startEdit(r.id, 'importe', r)} style={{ fontWeight: 700, cursor: 'pointer', borderBottom: `1px dotted ${custom ? '#b8952a' : '#ddd'}`, color: usd > 0 ? (custom ? '#b8952a' : '#1565a0') : '#ccc' }} title="Clic para editar importe">
                        {usd > 0 ? fmtUSD(usd) : '—'}
